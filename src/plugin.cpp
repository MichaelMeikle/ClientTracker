...
const char* ts3plugin_version() {
    return main->kPluginVersion.c_str();
}
...
const char* ts3plugin_author() {
	...
	return main->kPluginAuthor.c_str();;
}

/* Plugin description */
const char* ts3plugin_description() {
	/* If you want to use wchar_t, see ts3plugin_name() on how to use */
    return main->kPluginDescription.c_str();
}
 ...
 */
int ts3plugin_init() {
    return main->Initialize();  /* 0 = success, 1 = failure, -2 = failure but client will not show a "failed to load" warning */
...
void ts3plugin_initMenus(struct PluginMenuItem*** menuItems, char** menuIcon) {
	...
	//CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CLIENT,  MENU_ID_CLIENT_1,  "Client item 1", NULL);
